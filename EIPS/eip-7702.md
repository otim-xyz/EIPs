...
2. `authority = ecrecover(eip712_authorization_hash, y_parity, r, s)` (as defined in [EIP-712](./eip-712.md).)
...
Authorizations are signed using an [EIP-712](./eip-712.md) signature with the following parameters:

```python
DOMAIN_TYPE_HASH = keccak256('EIP712Domain(string name,string version,bytes32 salt)')

domain_separator = keccak256(
  DOMAIN_TYPE_HASH,
  keccak256('EIP7702Authorization')
  keccak256("v1"),
  keccak256("SET_EOA_ACCOUNT_CODE")
)

AUTHORIZATION_TYPE_HASH = keccak256('Authorization(uint256 chainId,uint64 nonce,address delegate)')

authorization_hash = keccak256(
  AUTHORIZATION_TYPE_HASH,
  chain_id,
  nonce,
  delegate_address
)

eip712_authorization_hash = keccak256('\x19\x01' || domain_separator || authorization_hash)
```
